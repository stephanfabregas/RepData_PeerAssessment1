...
The **mean** steps taken each day is `r mnsteps` and the **median** steps taken each day is `r mdsteps`.
...
The `r indintmax`th or interval occuring at time `r as.numeric(intmax)` contains the maximum number of steps, on average.
...
There are `r mvs` missing values in the data. Since there are only 3 variables in the dataset and `r nmvs` of the missing values come from the other two variables, all `r mvs` missing values come from the steps variable.
...
The **mean** steps taken each day is `r mv.mnsteps` and the **median** steps taken each day is `r mv.mdsteps`, once the missing values have been replaced with the mean value for that 5-minute interval.
